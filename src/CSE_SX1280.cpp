...
CSE_SX1280:: CSE_SX1280 (uint8_t cssPin, uint8_t busyPin, uint8_t resetPin) {
  sx1280CssPin = cssPin;
...
void CSE_SX1280:: zeroingAnArray (uint8_t arrayToZero[], uint16_t arrayLength ){
...
bool CSE_SX1280:: isBusy() {
  if (digitalRead (sx1280BusyPin) == HIGH) {
    return true;
  }
...
void CSE_SX1280:: rfSetup ( uint8_t standbyMode, 
                            uint8_t packetType, 
  ...
  *(setupWriteData + 1) = standbyMode; // Setting STDBY_RC Mode 0x01, STDBY_XOSC
  ...
  // Reassigning all array values to 0.
  zeroingAnArray (setupWriteData, 10);
  ...
  *( writeData + 4 ) = 0x00;
  sx1280Select();
  SPI.transfer( setupWriteData, 5*sizeof( uint8_t ) );
  sx1280Deselect(); 
...
void CSE_SX1280:: txSetup ( uint8_t power, 
                            uint8_t rampTime, 
                            uint8_t outboundMessage[],
                            uint8_t txIrq158, 
                            uint8_t txIrq70, 
    ...
    *(txWriteData + 2) = rampTime; // rampTime
    ...
    zeroingAnArray (txWriteData, 258);
    ...
    *(txWriteData + 3) = txPeriodBaseCount70;     // setting periodBaseCount bits 8 to 0 
    ...
    for (i = 0; i <= 100; i++) {
      ...
      *(txWriteData + 3) = 0x00;
      ...
      sx1280Select();
      // Arduino SPI replaces txWriteData with incoming data when size is specified.
      SPI.transfer (txWriteData, 4 * sizeof (uint8_t)); 
      sx1280Deselect();
...
void CSE_SX1280:: rxSetup ( uint8_t rxIrq158, 
                            uint8_t rxIrq70, 
    ...
    *(rxWriteData + 8) = 0x00;      /* setting DIO 3 Mask bits 7:0 to 0 */
    ...
    sx1280Select();
    SPI.transfer (rxWriteData, 9 * sizeof (uint8_t));
    sx1280Deselect();

    zeroingAnArray (rxWriteData, 258);
    ...
    *(rxWriteData + 3) = rxPeriodBaseCount70;   /* perdiodBase[7:0] for rx      */
    ...
    sx1280Select();
    SPI.transfer (rxWriteData, 4 * sizeof (uint8_t));
    sx1280Deselect();
      ...
      *(rxWriteData + 3) = 0x00;  /* IRQ bits 7 to 0 returned     */
      ...
      if ((*(rxWriteData + 3) & 0x02) == 0x02) { /* GETIRQSTATUS RxDone == 1 */
        ...
        *(rxWriteData + 6) = 0x00;
        ...
        sx1280Select();
        SPI.transfer (rxWriteData, 7 * sizeof (uint8_t));
        sx1280Deselect();
        ...
        *(rxWriteData + 2) = 0xFF;
        ...
        sx1280Select();
        SPI.transfer (rxWriteData, 3 * sizeof (uint8_t));
        sx1280Deselect();
    ...
    *(rxWriteData + 1) = 0x00;
    ...
    sx1280Select();
    SPI.transfer (rxWriteData, 2 * sizeof (uint8_t));
    sx1280Deselect();
